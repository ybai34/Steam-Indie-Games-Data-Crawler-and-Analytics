...
key_word = 'Slay the Spire'
# 先搜索得到id，通过搜索，得到游戏的id，id是采集后续的基础
@retry(tries=3)
def get_id(key_word, cookies1):
    params = {
        'a': 'all',
        'q': key_word,
    }
    key_word = key_word.lower()
    response1 = requests.get('https://steamdb.info/search/', params=params, cookies=cookies1, headers=headers)  # 基本请求语句
    myhtml1 = etree.HTML(response1.text)  # etree提取html结构，后续的就是通过xpath进行解析元素
    myid = myhtml1.xpath('//table[@id="table-sortable"]//tr[@class="app"]/@data-appid')
    name = myhtml1.xpath('//table[@id="table-sortable"]//tr[@class="app"]/td[3]/a')
    name = [''.join(name_i.xpath('.//text()')) for name_i in name]
    name = [name_i.lower() for name_i in name]
    try:
        myid_index =name.index(key_word)
    except Exception as e:
        myid_index = 0
        print(e)
    myid = myid[myid_index]
    return myid
...
# 获取前几列基础数据，主要就是request请求，lxml解析
@retry(tries=3)
...
# 开始执行，加载name，后期最好直接用id
key_df = pd.read_excel('Similar Games Data v1.02.xlsx')
key_word_list = key_df['Name'].to_list()
finish_key_word = []
...
# 核心代码，如果有报错，基本上就是被封ip了，这个时候，切换vpn，或者购买代理ip，关掉浏览器，执行上面打开浏览器的操作，再进行一遍就可以接触风控，就可以继续执行这段代码。
#
# 时刻观察，这个已完成的列表是否在更新，要确保采集过的在这个列表里，才能跳过
for key_word in tqdm(key_word_list):
    if key_word in finish_key_word:
        continue


    myid = get_id(key_word, cookies1)
    print(myid)
    ...
    with open('data_base.txt', 'a', encoding='gb18030') as f:
        one_data = [key_word, myid, followers, PlayTracker, Gamalytic, VG,
        ...
        f.write(one_data + '\n')
    f.close()
    finish_key_word.append(key_word)
...
finish_key_word = list(data_df['Name'].unique())
